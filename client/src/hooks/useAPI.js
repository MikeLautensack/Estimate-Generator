...
const useAPI = () => {
    ...
    const getCustomers = async (token) => {
        const response = await axios.get('http://localhost:5000/customers/get', { headers: { Authorization: `Bearer ${token}` } })
        setCustomers(response.data)
    }
       
    const getEstimates = async (token) => {
        const response = await axios.get('http://localhost:5000/estimates/get', { headers: { Authorization: `Bearer ${token}` } })
        setEstimates(response.data)
    }

    const addCustomer = async (token, customer) => {
        const response = await axios.post('http://localhost:5000/customers/add', customer, { headers: { Authorization: `Bearer ${token}` } })
    }

    const addEstimate = async (token, estimate) => {
        const response = await axios.post('http://localhost:5000/estimates/add', estimate, { headers: { Authorization: `Bearer ${token}` } })
    }
