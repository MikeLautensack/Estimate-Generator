...
const useAPI = () => {
    ...
    const updateCustomer = async (token, customer, _id) => {
        const response = await axios.put(`http://localhost:9000/customers/update/${_id}`, customer, { headers: { Authorization: `Bearer ${token}` }})
        console.log(response)
    }

    const updateEstimate = async (token, estimate, _id) => {
        const response = await axios.put(`http://localhost:9000/estimates/update/${_id}`, estimate, { headers: { Authorization: `Bearer ${token}` }})
        console.log(response)
    }

    const deleteCustomer = async (token, customer, _id) => {
        const response = await axios.delete(`http://localhost:9000/customers/delete/${_id}`, { headers: { Authorization: `Bearer ${token}` }})
        console.log(response)
    }

    const deleteEstimate = async (token, estimate, _id) => {
        const response = await axios.delete(`http://localhost:9000/estimates/delete/${_id}`, { headers: { Authorization: `Bearer ${token}` }})
        console.log(response)
    }
