    ...
    try {
        if (emailType == "new-customer") {
            await resend.emails.send({
                ...
                react: NewCustomerEmail({ url, host, customerName: fotmatedCustomerName as string, contractorName: fotmatedContractorName as string })
            });
            ...
            await resend.emails.send({
                ...
                to: [identifier],
                subject: `Log in to ${host}`,
                react: NewEstimateEmail({ url, host, customerName: fotmatedCustomerName as string, contractorName: fotmatedContractorName as string })
            });
            ...
            await resend.emails.send({
                ...
                to: [identifier],
                subject: `Log in to ${host}`,
                react: UpdatedEstimateEmail({ url, host, customerName: fotmatedCustomerName as string, contractorName: fotmatedContractorName as string })
            });
        }
    } catch (error) {
