...
describe("AuthUseCases", () => {
  ...
  beforeEach(() => {
    ...
    // Create the AuthUseCases instance with the mocked service
    authUseCases = new AuthUseCases(mockSupabaseService);
  });

  describe("getServerClient", () => {
    it("should return the server client from supabase service", async () => {
      // Arrange
      mockSupabaseService.getServerClient.mockResolvedValue(mockClient);

      // Act
      const result = await authUseCases.getServerClient();
    ...
    it("should handle errors from supabase service", async () => {
      // Arrange
      const error = new Error("Database connection failed");
      mockSupabaseService.getServerClient.mockRejectedValue(error);

      // Act & Assert
      await expect(authUseCases.getServerClient()).rejects.toThrow(
  ...
  describe("getSession", () => {
    it("should get session using server client", async () => {
      // Arrange
      ...
      // Act
      const result = await authUseCases.getSession();

      // Assert
      expect(mockSupabaseService.getServerClient).toHaveBeenCalledTimes(1);
      expect(mockSupabaseService.getSession).toHaveBeenCalledWith(mockClient);
    ...
    it("should handle errors when getting session", async () => {
      ...
      // Act & Assert
      await expect(authUseCases.getSession()).rejects.toThrow(
  ...
  describe("getUser", () => {
    it("should get user using server client", async () => {
      // Arrange
      const mockUser = {
        id: "123",
      ...
      // Act
      const result = await authUseCases.getUser();

      // Assert
      expect(mockSupabaseService.getServerClient).toHaveBeenCalledTimes(1);
      expect(mockSupabaseService.getUser).toHaveBeenCalledWith(mockClient);
    ...
    it("should handle errors when getting user", async () => {
      ...
      // Act & Assert
      await expect(authUseCases.getUser()).rejects.toThrow(
  ...
  describe("updateSession", () => {
    it("should update session using supabase service", async () => {
      // Arrange
      ...
      // Act
      const result = await authUseCases.updateSession(mockRequest);

      // Assert
      expect(mockSupabaseService.updateSession).toHaveBeenCalledWith(
        mockRequest,
      );
      expect(result).toBe(mockResponse);
    });

    it("should handle errors when updating session", async () => {
      // Arrange
      const mockRequest = new NextRequest(
        "http://localhost:3000/api/auth/callback",
      );
      ...
      // Act & Assert
      await expect(authUseCases.updateSession(mockRequest)).rejects.toThrow(
